...
from app.forms import FormLogin, FormCriarConta, FormEditarPerfil, FormAcoes, FormCompras, FormVendas, FormApuracao, FormBusca
from app.models import Usuario, Acoes, Compras, Vendas, Apuracao
...
def compras_lista():
    ...
    for compra in compras:
        #compra.data = datetime.datetime.fromtimestamp(compra.data).data.('%d/%m/%Y')
...
#@app.route('/cotacao')
#def cotacao():
#    url = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=PETR3.SA&apikey=KSXJGBLP8S2N3H13'
#    r = requests.get(url)
#    cot_acao = r.json()

    #print(cot_acao)

    #return render_template('cotacao.html', cot_acao=cot_acao)
...
def busca():
    ...
    if form_busca.validate_on_submit():
        busca_acao = form_busca.data.data
        
        
        url = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=' + busca_acao + '.SA&apikey=KSXJGBLP8S2N3H13'
        r = requests.get(url)
        cot_acao = r.json()
